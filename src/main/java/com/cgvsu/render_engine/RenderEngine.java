...
public class RenderEngine {
    ...
    public static void render(final GraphicsContext graphicsContext,
        ...
        Matrix4X4 modelMatrix = rotateScaleTranslate(mesh.getScale().getData(0), mesh.getScale().getData(1),
                                                     mesh.getScale().getData(2), mesh.getRotation().getData(0),
                                                     mesh.getRotation().getData(1), mesh.getRotation().getData(2),
                                                     mesh.getTranslation().getData(0), mesh.getTranslation().getData(1),
                                                     mesh.getTranslation().getData(2));
        ...
        for (int i = 0; i < mesh.polygons.size(); ++i) {
            ...
            Point2f p1 = vertexToPoint(v1N, width, height);
            Point2f p2 = vertexToPoint(v2N, width, height);
            Point2f p3 = vertexToPoint(v3N, width, height);
            
            Rasterizator.rasterizeTriangle(graphicsContext, ZBuffer, v1, v2, v3, p1, p2, p3, Color.BLUE, width);
